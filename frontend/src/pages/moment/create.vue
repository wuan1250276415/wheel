...
const submitting = computed(() => momentStore.isCreating)

const canSubmit = computed(() => {
  return formData.value.content.trim().length > 0 && !uploading.value
})
...
// 取消
function onCancel() {
  if (formData.value.content.trim() || formData.value.images.length > 0) {
    uni.showModal({
      title: '提示',
      content: '确定要放弃编辑吗？',
      success: (res) => {
...
async function onSubmit() {
  ...
  const success = await momentStore.createMoment({
