...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='情侣关系表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='转盘分类表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='转盘内容表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='转盘配置表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='用户行为统计表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='内容审核日志表';
...
-- 插入默认分类数据
INSERT INTO `wheel_category` (`category_name`, `description`, `sort_order`, `icon_url`, `theme_color`, `status`,
                              `is_system`)
VALUES ('聊天话题', '增进了解的有趣话题', 100, '/icons/chat.png', '#FF6B9D', 1, 1),
...
-- 插入示例转盘内容（聊天话题）
INSERT INTO `wheel_content` (`category_id`, `content_text`, `weight`, `is_system`, `audit_status`)
VALUES (1, '今天最开心的事情是什么？', 1.0, 1, 1),
...
BEGIN
    ...
    -- 检查邀请码是否已存在
    WHILE EXISTS (SELECT 1 FROM couple_relationship WHERE invite_code = result)
        DO
            SET i = 0;
            SET result = '';
            WHILE i < len
...
SELECT u.id                                                    AS user_id,
       ...
       COUNT(CASE WHEN r.create_time >= CURDATE() THEN 1 END) AS today_spins,
...
(
    ...
    `message_type` tinyint(1)          DEFAULT 1 COMMENT '消息类型：1-文字，2-图片，3-转盘结果，4-系统消息',
    `content`      text COMMENT '消息内容',
    `extra_data`   json                DEFAULT NULL COMMENT '扩展数据（图片URL、转盘结果等）',
    `is_read`      tinyint(1)          DEFAULT 0 COMMENT '是否已读：0-未读，1-已读',
    `read_time`    datetime            DEFAULT NULL COMMENT '阅读时间',
    `status`       tinyint(1)          DEFAULT 1 COMMENT '消息状态：0-已删除，1-正常',
    `create_time` datetime   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`  datetime   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`      tinyint(1)          DEFAULT 1 COMMENT '逻辑删除标志：0-已删除，1-未删除',
    PRIMARY KEY (`id`),
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='用户在线状态表';
...
(
    ...
    message_type TINYINT  NOT NULL DEFAULT 1 COMMENT '消息类型：1-文字，2-图片，3-转盘结果，4-系统消息',
    content      TEXT COMMENT '消息内容',
    extra_data   JSON COMMENT '扩展数据（图片URL、转盘结果等）',
    is_read      TINYINT  NOT NULL DEFAULT 0 COMMENT '是否已读：0-未读，1-已读',
    read_time    DATETIME COMMENT '阅读时间',
    status       TINYINT  NOT NULL DEFAULT 1 COMMENT '消息状态：0-已删除，1-正常',
    `create_time`  datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`   datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `create_by`    bigint(20)            DEFAULT 0 COMMENT '创建人',
    `update_by`    bigint(20)            DEFAULT 0 COMMENT '更新人',
    INDEX idx_couple_time (couple_id, create_time DESC),
    INDEX idx_receiver_read (receiver_id, is_read, create_time DESC)
) ENGINE = InnoDB
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='用户在线状态表';
